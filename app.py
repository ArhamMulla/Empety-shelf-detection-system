...
def start_capture(client_id):
    ...
    camera = Camera.query.filter_by(id=client_id).first()
    if not camera:
        return jsonify({'error': 'Camera not found'}), 404

    # Prepare payload with camera details
    response_payload = {
...
@app.route('/get_threshold/<client_id>', methods=['GET'])
def get_threshold(client_id):
    camera = Camera.query.filter_by(id=client_id).first()
    if camera:
        return jsonify({'threshold': camera.threshold})
    return jsonify({'error': 'Camera not found'}), 404

@app.route('/set_threshold/<client_id>', methods=['POST'])
def set_threshold(client_id):
    data = request.json
    camera = Camera.query.filter_by(id=client_id).first()
    if camera and 'threshold' in data:
        camera.threshold = data['threshold']
        db.session.commit()
        return jsonify({'status': 'Threshold updated'}), 200
    return jsonify({'error': 'Threshold value not provided or Camera not found'}), 400
...
def get_images(client_id):
    ...
    for img_file in sorted(os.listdir(client_folder)):
        if img_file.endswith(".jpg"):
            ...
            message = f"Empty space detected! by shelf {client_id}"
            images.append({
                "cid": client_id,
